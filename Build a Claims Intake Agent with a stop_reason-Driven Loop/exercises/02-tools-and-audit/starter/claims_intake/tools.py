...
TOOL_SCHEMAS: list[dict[str, Any]] = [
    {
        "name": "lookup_policy",
        "description": (
            "Look up the policy record to confirm coverage, deductible, and policy status early in the "
            "claim. Use this before classifying or routing."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "policy_id": {"type": "string", "description": "Policy identifier, for example POL-1001"}
            },
            "required": ["policy_id"],
        },
    },
    {
        ...
        "description": (
            "Record one normalized fact from the claimant's statement, such as incident_date, location, "
            "items_lost, description, or injury_party. Call this once per distinct fact."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "field": {"type": "string", "description": "Snake_case fact name"},
                "value": {"type": "string", "description": "Fact value as a short string"},
            },
    ...
    },
    {
        "name": "classify_claim",
        "description": (
            "Commit to a single claim type with a confidence score and rationale. Use this exactly once "
            "after enough facts are gathered."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "confidence": {
                    ...
                    "minimum": 0.0,
                    "maximum": 1.0,
                    "description": "Confidence in [0, 1] that the chosen claim type is correct",
                },
                "rationale": {"type": "string", "description": "Brief rationale for the classification"},
            },
    ...
    },
    {
        ...
        "description": (
            "Commit to a severity bucket after classification. This reflects damage magnitude, injury detail, "
            "and policy exposure."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "rationale": {"type": "string", "description": "Brief rationale for the severity choice"},
            },
    ...
    },
    {
        ...
        "description": (
            "Ask the claimant one clarifying question when the claim type is genuinely ambiguous. Use "
            "ambiguity_between to name candidate types. Return the scripted claimant answer if available; "
            "otherwise return NO_RESPONSE."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "question": {"type": "string", "description": "Question to ask the claimant"},
                "ambiguity_between": {
                    "type": "array",
                    "items": {"type": "string", "enum": CLAIM_TYPES},
                    "description": "Candidate claim types that are unresolved by the current facts",
                },
            },
    ...
    },
    {
        ...
        "description": (
            "Terminal tool: route a claim to the appropriate adjuster queue when confidence is high enough and "
            "the severity has been assessed."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "queue": {"type": "string", "enum": CLAIM_TYPES},
                "claim_summary": {"type": "string", "description": "One-paragraph summary of the claim"},
            },
    ...
    },
    {
        ...
        "description": (
            "Terminal tool: escalate the case to a human adjuster when confidence is too low, the claim is "
            "ambiguous, or routing would be unsafe."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
...
def _t_request_clarification(session: ClaimSession, inp: dict[str, Any]) -> str:
    ...
    if not isinstance(ambiguity_between, list) or len(ambiguity_between) < 2:
        return _err("permanent", False, "ambiguity_between must be a list with at least 2 items")
...
def _t_route_to_adjuster(session: ClaimSession, inp: dict[str, Any]) -> str:
    if session.terminal_called:
        return _err("permanent", False, "terminal tool already called")
    queue = inp.get("queue")
    ...
    path = session.run_dir / "queues" / f"{queue}.jsonl"
...
def _t_escalate_to_human(session: ClaimSession, inp: dict[str, Any]) -> str:
    ...
    missing = [name for name in required if name not in structured_summary]
    if missing:
        return _err("permanent", False, f"structured_summary missing required fields: {', '.join(missing)}")
    ...
    record = {
        ...
        "reason": reason,
        **structured_summary,
        "case_facts_at_escalation": dict(structured_summary["case_facts"]),
    }
    session.escalation = record
    path = session.run_dir / "escalations.jsonl"
    _append_jsonl(path, record)
